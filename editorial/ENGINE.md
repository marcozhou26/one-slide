...
3. 写出至少两项具体页面优势，并标为 Builder 必须保护的基线；
4. 判断是否存在一个足以改变读者理解或明显降低完成度的材料性问题；
5. 输出 `editorial-qa.json`，运行 `scripts/validate_editorial_qa.py`；
6. `PASS_AS_IS` 直接结束，禁止生成 Builder revision plan；
7. `BUILDER_LOCAL_REPAIR` 或 `BUILDER_RECOMPOSE` 只把一条任务交给 Builder。
