...
3. 按六维 rubric 写 `editorial-diagnosis.json`，只选择一个主问题；
4. 没有材料性问题时复制为版本化输出，记录 `NO_MATERIAL_EDIT`；
5. 有明确假设时写 `editorial-patch.json`，再运行 `scripts/apply_editorial_patch.mjs`；
6. 重新 inspect 候选，用前后 `inspect-manifest.json` 运行 `scripts/verify_editorial_roundtrip.py`，禁止使用未与 PPTX 哈希绑定的图片；
7. 对候选执行 OneSlide layout、信息贡献、语义、溢出、`scripts/audit_editorial_contrast.py` 和 Microsoft PowerPoint 实际打开/编辑检查；任一门槛失败即回退；
