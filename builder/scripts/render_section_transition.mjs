...
export const renderSectionTransition = (data, output) => renderStructurePage(data, output, "section-transition");
async function main() { const options = parseCliArgs(process.argv.slice(2)); const data = await loadStructureInput(options.input, "section-transition"); await renderSectionTransition(data, options); process.stdout.write(`${JSON.stringify({ ok: true, module: data.module_id })}\n`); }
