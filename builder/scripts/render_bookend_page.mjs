...
export const renderBookendPage = (data, output) => renderStructurePage(data, output, "bookend-page");
async function main() { const options = parseCliArgs(process.argv.slice(2)); const data = await loadStructureInput(options.input, "bookend-page"); await renderBookendPage(data, options); process.stdout.write(`${JSON.stringify({ ok: true, module: data.module_id })}\n`); }
