...
function renderRail(slide, plan, diagram) {
  ...
  addTextBox(slide, { name: "insight-title", text: "Conclusion of picture reading", position: { left: plan.rail.left + 16, top: plan.rail.top + 16, width: plan.rail.width - 32, height: 28 }, fontSize: 18, bold: true, color: COLORS.navy, singleLine: true });
...
export async function renderCohortRetention(data, output) {
  ...
  addTextBox(slide, { name: "cohort-definition", text: `Batch:${d.cohort_definition.text}`, position: { left: plan.chart.left + 18, top: metaTop, width: 250, height: 40 }, fontSize: 14, bold: true, color: COLORS.navy, maxLines: 2 });
  addTextBox(slide, { name: "measure-definition", text: `Indicators:${d.measure.text}`, position: { left: plan.chart.left + 278, top: metaTop, width: 390, height: 40 }, fontSize: 14, color: COLORS.muted, maxLines: 2 });
  addTextBox(slide, { name: "denominator-definition", text: `Denominator:${d.denominator.text}`, position: { left: plan.chart.left + 678, top: metaTop, width: 190, height: 40 }, fontSize: 14, color: COLORS.muted, alignment: "right", maxLines: 2 });
  ...
  addTextBox(slide, { name: "x-axis-unit", text: `Relative period (${d.relative_period_unit.text})`, position: { left: plot.left + plot.width - 154, top: plot.top + plot.height + 31, width: 154, height: 20 }, fontSize: 12, color: COLORS.muted, alignment: "right", singleLine: true });
  ...
  d.cohorts.forEach((cohort, index) => {
    ...
    const matureLabel = lastIndex === d.relative_periods.length - 1 ? "Observed completely" : `observed to${d.relative_periods[lastIndex].label.text}`;
