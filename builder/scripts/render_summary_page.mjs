...
export const renderSummaryPage = (data, output) => renderStructurePage(data, output, "summary-page");
async function main() { const options = parseCliArgs(process.argv.slice(2)); const data = await loadStructureInput(options.input, "summary-page"); await renderSummaryPage(data, options); process.stdout.write(`${JSON.stringify({ ok: true, module: data.module_id })}\n`); }
