...
function renderRail(slide, plan, d, calculated) {
  ...
  d.insights.forEach((item, index) => {
    addTextBox(slide, { name: `insight-index-${index + 1}`, text: String(index + 1).padStart(2, "0"), position: { left: plan.rail.left + 16, top: plan.rail.top + 142 + index * 76, width: 34, height: 22 }, fontSize: 14, bold: true, color: index === 0 ? COLORS.orange : COLORS.blue, singleLine: true });
    addTextBox(slide, { name: `insight-${index + 1}`, text: item.text, position: { left: plan.rail.left + 56, top: plan.rail.top + 136 + index * 76, width: plan.rail.width - 72, height: 60 }, fontSize: 16, bold: index === 0, color: COLORS.text, verticalAlignment: "top", maxLines: 3 });
  });
