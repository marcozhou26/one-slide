...
const blindCases = [
  ...
  ["hr-operating-diagnostic-matrix", { input_mode: "text", text: "把五类HR服务按需求量、一次解决率、积压和责任组做运营诊断矩阵" }],
];
...
test("retired fixed templates route to direct-composition patterns", async () => {
  const cases = [
    ["route-tradeoff", "比较自建和外采两条路线，按成本、周期和风险做取舍"],
    ["scqa-roadmap", "用SCQA说明情境、冲突、问题、答案，再接三阶段落地路径"],
    ["scenario-planning", "比较悲观、基准、乐观三种情景和无悔举措"],
    ["industry-value-chain", "按上游、中游、下游比较参与者覆盖和利润率"],
    ["spiral-maturity", "用四圈螺旋表达设计、执行、度量、沉淀的成熟升级"],
  ];
  for (const [pattern, text] of cases) {
    const result = await routeInput({ text });
    assert.equal(result.decision, "direct_composition", JSON.stringify(result));
    assert.equal(result.preferred_pattern, pattern, JSON.stringify(result));
  }
});

test("retired map module is blocked and legacy HR templates converge on one matrix", async () => {
  await assert.rejects(
    () => routeInput({ text: "按城市在地图上放区域气泡并连接明细表" }),
    (error) => error.code === "SENSITIVE_MAP_MODULE_RETIRED",
  );
  for (const text of [
    "用职级和职能序列做双维格子，检查管理跨度和人数",
    "整理HR服务目录，逐项列需求量、成功率和自动化覆盖",
    "比较受理渠道的工单量、一次解决率与积压",
  ]) {
    const result = await routeInput({ text });
    assert.equal(result.module.module_id, "hr-operating-diagnostic-matrix", JSON.stringify(result));
  }
});

test("retired modules cannot be requested as productized modules", async () => {
