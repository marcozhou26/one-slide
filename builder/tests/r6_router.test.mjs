...
const blindCases = [
  ...
  ["small-multiples", { input_mode: "text", text: "七条业务线用同一收入指数比较2022到2026趋势，每格重复一条迷你折线，统一刻度和公司基准，并按投资、维持、观察、退出状态分类" }],
...
test("retired HR specialization modules cannot be requested or inferred", async () => {
  ...
  await assert.rejects(() => routeInput({ text: "按资格条件和覆盖人群核对政策是否适用及例外情形，形成资格覆盖矩阵" }), (error) => error.code === "ROUTE_EVIDENCE_INSUFFICIENT");
  await assert.rejects(() => routeInput({ text: "按薪酬、招聘、员工关系等工单分类统计重分类率和最终分类" }), (error) => error.code === "ROUTE_EVIDENCE_INSUFFICIENT");
});

test("retired modules and geographic maps are blocked explicitly", async () => {
  await assert.rejects(
    () => routeInput({ text: "按省市行政区绘制全国分布地图并放气泡" }),
    (error) => error.code === "MAP_POLITICAL_RISK_BLOCKED",
  );
  await assert.rejects(
    () => routeInput({ requested_module: "region-map-table" }),
    (error) => error.code === "MAP_POLITICAL_RISK_BLOCKED",
  );
  for (const requested_module of ["spiral-maturity", "hr-new-hire-survival"]) {
    await assert.rejects(
      () => routeInput({ requested_module }),
      (error) => error.code === "MODULE_RETIRED",
    );
  }
});

test("new-hire retention now routes to the general cohort module", async () => {
  const result = await routeInput({
    input_mode: "text",
    text: "四个招聘渠道按入职批次，从第0个月比较第3、6、12、24个月仍在职人数；较新批次后续月份尚未观察，空白不能当0",
  });
