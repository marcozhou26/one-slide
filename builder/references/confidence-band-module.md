...
- `source_note`：页面底部唯一可见的简短数据来源；期间与口径进入备注；
...
页面使用 PowerPoint 原生折线、圆点、坐标轴、虚线阈值，以及相邻时期之间的可编辑半透明四边形估计范围。颜色不是唯一通道：中心估计用实线和圆点、估计范围用填充带、阈值用虚线并附普通中文标签。右侧只保留最多两条洞察和克制结论；区间正式名称、定义、估计方法、样本/总体、缺失说明和阈值语义进入 PowerPoint 备注。页底只显示数据来源，合成披露并入该行。
