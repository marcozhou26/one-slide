...
| `flows` | required | 仅连接相邻层；value>0；`kind` 为 `neutral`、`on_time`、`not_on_time`，并兼容历史 `success`、`loss` | 未知语义、跨层连接、端点缺失或层间不守恒时停止；不得用“其他”补差 | validator → planner `flow_semantics` → renderer 线型与颜色 |
...
- `neutral`：中性灰蓝实线，不暗示成功或损耗。
- `on_time`：蓝色实线，表示按时完成。
- `not_on_time`：橙色虚线，标签和对象名保留“未按时”，不得改写为“损耗”。
- 历史 `success/loss` 继续使用蓝色实线/橙色虚线，不改变既有含义。
- 五行 SLA 核验区必须是一个局部表格式区域，完整保留服务量、FTE、准时率、SLA 状态四字段，不能拆成五张卡。
- 节点与流量带均使用原生可编辑对象；上下镜像分流无法避让时，用白色断点和上层连续流形成桥接，不能让颜色在交点融合。
- 流线必须位于白色主框之上、节点之下；三类流向使用足够的对比度和线宽，使流向成为页面主视觉。每层标题使用完整列宽，不得用节点宽度充当标题宽度。

## QA

- 核对每个节点值、每层合计和相邻层流量守恒。
- 核对 `neutral/on_time/not_on_time` 的对象命名、颜色和线型；不得出现把“未按时”写成“损耗”的可见文本。
- 核对 SLA 五行四字段、线宽排序、桥接断点、连接器层级和局部表格可读性，并在 PowerPoint 中实开。
- OOXML 中存在 42 条连接器不能单独证明可读；整页预览和 PowerPoint 常规视图都必须能识别中性流、按时流与未按时流，层标题不得挤断数字或被节点遮挡。
