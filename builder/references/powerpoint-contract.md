...
- 输出必须包含 `.json` layout，并通过 `layout_quality.mjs`；无信息贡献的装饰对象、换行孤字、英文追踪 key 拆词、两行标题叠加副标题、数字单位拆分、跨区块边缘错位、画布利用不足和对象越界均阻断交付。
- 模板也必须通过 `information-contribution-gate.md`；不能因为模板原有形状而保留顶部装饰带、eyebrow、标题饰线或空装饰容器。
