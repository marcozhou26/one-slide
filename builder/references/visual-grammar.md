...
- 先区分结构层级、稳定可读性规则和业务强调。前两类用于帮助阅读且规则固定；第三类必须有来源支持。无法归类的差异一律删除。
- 主色不超过三种；异常暖色、达标冷色、中性灰。
- 主证据占最大空间；解释和动作不得抢占主证据权重。
- 通过位置、字号、字重、留白和少量色彩建立层级，不使用装饰图标、阴影、渐变和网页式组件。只有导航、对象、状态、动作或少量流程节点确实需要语义标识时，才按 `semantic-icon-library.md` 使用精选 SVG；不得填空、编码数据或制造业务强调。
- 同级内容默认完全同样式。`allow_random_bold=false`、`allow_random_color=false`、`allow_random_fill=false`、`allow_random_border=false`；禁止用 `index === 0`、固定 `rowIndex`、固定 `columnIndex` 或数组位置决定业务强调。允许中性奇偶行/列底色帮助阅读，但不得改变字重、文字色、边框、字号或业务语义。
- 强调必须来自完整的 `allowed_emphasis`（目标、业务理由、方法、来源）。没有授权时，除标题与表头的结构层级外，正文全部使用常规字重和统一中性色。
- 业务强调只使用加粗与底纹。禁止用边框颜色、边框粗细、边框线型或缺边表达强调；浅色底纹不改变文字色。相邻容器的共边必须保持统一细线。
